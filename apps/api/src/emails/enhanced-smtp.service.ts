...
export class EnhancedSmtpService {
  ...
  async createTransporter(config: SmtpConfigData): Promise<nodemailer.Transporter> {
    ...
    if (config.enableDkim && config.dkimPrivateKey && config.dkimSelector && config.dkimDomain) {
    ...
    try {
      ...
      this.logger.log(`SMTP transporter created successfully for ${config.host}:${config.port}`);
      return transporter;
    } catch (error) {
  ...
  async sendEmailWithConfig(
    ...
    try {
      ...
      this.logger.log(`Email sent successfully via SMTP to ${emailData.to}. MessageId: ${info.messageId}, DeliveryTime: ${deliveryTime}ms`);
  ...
  private async validateSenderConfiguration(config: SmtpConfigData): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(`Sender configuration validation warning: ${error.message}`);
    }
  }
  ...
  private async checkDomainReputation(fromEmail: string): Promise<string[]> {
    ...
    try {
      ...
      if (!spfRecord) {
        warnings.push(`No SPF record found for domain ${domain}. This may affect deliverability.`);
      }
      
      // Check for DMARC record
      const dmarcRecord = await this.lookupDMARCRecord(domain);
      if (!dmarcRecord) {
        warnings.push(`No DMARC record found for domain ${domain}. Consider adding one for better reputation.`);
      }
    } catch (error) {
  ...
  private async lookupSPFRecord(domain: string): Promise<string | null> {
    ...
    try {
      ...
      return spfRecord ? (Array.isArray(spfRecord) ? spfRecord.join('') : spfRecord) : null;
    } catch (error) {
      return null;
    }
  }
  ...
  private async lookupDMARCRecord(domain: string): Promise<string | null> {
    ...
    try {
      ...
      return dmarcRecord ? (Array.isArray(dmarcRecord) ? dmarcRecord.join('') : dmarcRecord) : null;
    } catch (error) {
      return null;
    }
  }
  ...
  private async checkRateLimit(configKey: string): Promise<void> {
    ...
    // Clean old entries
    stats.recentSends = stats.recentSends.filter((time: number) => time > windowStart);
  ...
  private updateConnectionStats(configKey: string, success: boolean, deliveryTime: number): void {
    const stats = this.connectionStats.get(configKey);
    if (!stats) return;
    ...
    if (success) {
      ...
      stats.averageDeliveryTime = 
        (stats.averageDeliveryTime * (stats.totalSent - 1) + deliveryTime) / stats.totalSent;
    } else {
      stats.totalFailed++;
  ...
  private calculateReputationScore(configKey: string): number {
    ...
    const deliveryScore = Math.min(100, 100000 / (stats.averageDeliveryTime || 1000));
